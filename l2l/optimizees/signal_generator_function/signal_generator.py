...
SignalGeneratorOptimizeeParameters = namedtuple('SignalGeneratorOptimizeeParameters',
                                                ['frequency', 'amplitude', 'phase', 'seed', 'range'])
...
class SignalGeneratorOptimizee(Optimizee):
    ...
    def bounding_func(self, individual):
        return \
            {'amp': np.clip(-2, 2, individual['amp']), 'phase': np.clip(-1, 1, individual['phase'])}
    ...
    def simulate(self, traj):
        ...
        # return self.mean_square_erorr_list(y_pred=y_pred, y_true=y_real)

    def generate_signal(self, amplitude, phase, freq, time=np.arange(0, 1, 1.0 / 100.0)):
