...
"              d_nodes2output)"
  ...
  create_nest_agent 0 1 2 4 3 1 0 ;activator[0] activator[1] activator[2] activator[4] activator[3] spike_detector[1] spike_detector[0]
end

to create_nest_network_old4
  (py:run
"import nest"
"import numpy as np"
"import pandas as pd"
...
"                             syn_spec=syn_spec_nodes)"
"                i += 1"
"    # nest.Connect(nodes, nodes, conn_dict)"
"def connect_manually(sources, targets, weights, delays):"
"    i = 0"
"    for source in range(len(sources)):"
"        for target in range(len(targets)):"
...
"    1, sim_time, int(sim_time/heartbeat_interval * dt)).round()})"
"# Activator activates the input neurons, will be manipulated by NetLogo"
"activator = nest.Create('dc_generator', 5, params={'amplitude': 400.})"
"rng = np.random.default_rng(0)"
"# Create nodes"
"params = {'t_ref': 1.0}"
"inpt = nest.Create('iaf_psc_alpha', 5, params=params)"
...
"connect_synapses()"
"csv = pd.read_csv('individual_config.csv', header=None, na_filter=False)"
"weights = csv.iloc[0].values * 1"
"delays = csv.iloc[1].values"
"w_input2nodes = weights[:30].reshape(6, 5)"
"w_heartbeat2input = weights[30:36].reshape(6, 1)"
"w_nodes2nodes = weights[36:66]"
"w_nodes2output = weights[66:].reshape(2, 6)"
"d_input2nodes = delays[:30].reshape(6, 5)"
"d_heartbeat2input = delays[30:36].reshape(6, 1)"
"d_nodes2nodes =  delays[36:66]"
"d_nodes2output = delays[66:].reshape(2, 6).astype(float)"
"connect_nodes(w_input2nodes, d_input2nodes, w_heartbeat2input,"
"              d_heartbeat2input, w_nodes2nodes, d_nodes2nodes, w_nodes2output,"
"              d_nodes2output)"
"nest.Simulate(sim_time)"
"# print(nest.GetStatus(middle_spike_detector[2]))"
"print(nest.GetStatus(spike_detector[0]))"
"# print(nest.GetStatus(input_detector[2]))"
  )
  print "created network!"
  create_nest_agent 0 1 2 4 3 1 0 ;activator[0] activator[1] activator[2] activator[4] activator[3] spike_detector[1] spike_detector[0]
;[ #visual_input_neuron_white #visual_input_neuron_red #visual_input_neuron_green #rewarding_input_neuron  #nociceptive_input_neuron #move_actuator_neuron  #rotate_actuator_right_neuron ]
end
...
"                 syn_spec=syn_spec_dict_heartbeat)"
"    nest.Connect(inpt, nodes, conn_dict, syn_spec=syn_spec_dict_input)"
"    # nest.Connect(nodes, nodes, conn_dict, syn_spec=syn_spec_dict_nodes)"
"    nest.Connect(nodes, outpt, conn_dict, syn_spec=syn_spec_dict_output)"
"    nest.Connect(outpt, spike_detector)"
"    for source in range(6):"
"        for target in range(6):"
"            if source != target:"
"                w = nodes_w[source * 5 + target]"
"                d = nodes_d[source * 5  + target]"
"                syn_spec_nodes = {'weight': w, 'delay': d}"
"                nest.Connect(nodes[source], nodes[target], 'one_to_one',"
"                             syn_spec=syn_spec_nodes)"
"    nest.Connect(nodes, nodes, conn_dict)"
...
"    1, sim_time, int(sim_time/heartbeat_interval * dt)).round()})"
"# Activator activates the input neurons, will be manipulated by NetLogo"
"activator = nest.Create('dc_generator', 5)"
"rng = np.random.default_rng(0)"
"# Create nodes"
"inpt = nest.Create('iaf_psc_alpha', 5)"
"nodes = nest.Create('iaf_psc_alpha', 6)"
"outpt = nest.Create('iaf_psc_alpha', 2)"
"spike_detector = nest.Create('spike_recorder', 2)"
"middle_spike_detector = nest.Create('spike_recorder', 6)"
"# connect spike generator to input"
"nest.Connect(activator, inpt, 'one_to_one')"
...
"d_nodes2output = delays[66:].reshape(2, 6).astype(float)"
"ids = []"
"for idx, i in enumerate(range(12, 18)):"
"    ids.append([])"
"    for j in range(12, 18):"
"        if i != j:"
"            ids[idx].append(j)"
"nodes_ww = range(12, 18)"
"connect_nodes(w_input2nodes, d_input2nodes, w_heartbeat2input, d_heartbeat2input,"
"              w_nodes2nodes, d_nodes2nodes, w_nodes2output, d_nodes2output)"
"nest.Simulate(sim_time)"
"print(nest.GetStatus(middle_spike_detector[0]))"
 )
end


to create_nest_network_old
...
"def connect_synapses():"
"    # Connect synapses"
"    nest.CopyModel('static_synapse', 'static_synapse')  # syn_spec=syn_spec)"
"nest.ResetKernel()"
)
...
"dt = 1."
"heartbeat_interval = 2."
"heartbeat = nest.Create('spike_generator', spike_times=np.linspace(0, sim_time, int(sim_time*heartbeat_interval/dt)))"
"weights = np.random.randn(48)"
"delays = np.random.randint(48)"
"# Create nodes"
"inpt = nest.Create('iaf_psc_alpha', 5)"
"nodes = nest.Create('iaf_psc_alpha', 6)"
"outpt = nest.Create('iaf_psc_alpha', 2)"
...
"connect_synapses()"
"# TODO insert weights and delays"
"connect_nodes()"
"nest.simulate(sim_time)"
)
...
to compute-creature-fitness
  if last_nest_collision > last_food_collision and last_food_collision > 0 [
    set fitness fitness + ( 15 + ( 100 / (last_nest_collision - last_food_collision))  )
    set last_nest_collision 0
    set last_food_collision 0
  ]
end
...
to perceive-world ;;Called by testcreatures
 ...
 [
    ...
    [
      ...
      set_nest_dc_generator 3 input_dc_amplitude
      update-fitness -5.5
      if (istrainingmode?)
      [
    ...
    [
        ...
        set_nest_dc_generator 4 input_dc_amplitude
...
to do-actuators ;;Called by Creature
 ...
 let rotation_counter get_nest_spike_recorder 0
 let move_counter get_nest_spike_recorder 1
 ifelse rotation_counter > move_counter
 [
    set dorotation_right? true
    ;print "Rotate"
 ]
 [
    if move_counter > rotation_counter [ set domovement? true ]
 ]
 ...
 ;;if ( get_nest_spike_recorder 1 ) > 0
 ;;[
 ;;   set domovement? true
 ;;]
...
to propagate-visual-stimuli ;;Called by visual sensor
  ...
  [
     ...
     set_nest_dc_generator attached_to_neuron input_dc_amplitude;100000
  ]
...
to build_neural_circuit [ #insect_offset #weight_data #plasticity_data #delay_data ]
  ...
  repeat 12 [
    ...
    repeat 10 [
      ...
      setup-synapse  (#insect_offset + 11 + layer_from)  (#insect_offset + 200 + layer_to) abs(current_weight) type_exc_or_inh_synapse (round current_delay) plasticity?
      set current_index current_index + 1
  ...
  repeat 10 [
    ...
    repeat 10 [
      ...
      if layer_from != layer_to [
        setup-synapse  (#insect_offset + 200 + layer_from)  (#insect_offset + 200 + layer_to) abs(current_weight) type_exc_or_inh_synapse (round current_delay) plasticity?
        set current_index current_index + 1
      ]
      set layer_to layer_to + 1
    ]
  ...
  repeat 10 [
    ...
    repeat 4 [
      ...
      setup-synapse  (#insect_offset + 200 + layer_from)  (#insect_offset + 300 + layer_to) abs(current_weight) type_exc_or_inh_synapse (round current_delay) plasticity?
      set current_index current_index + 1
      set layer_to layer_to + 1
    ]
    set layer_from layer_from + 1
...
to create_nest_agent [ #visual_input_neuron_white #visual_input_neuron_red #visual_input_neuron_green #rewarding_input_neuron  #nociceptive_input_neuron #move_actuator_neuron  #rotate_actuator_right_neuron ]
  ;print "entered function to create creature"
  let creatureid create-creature 82 30 1 #rewarding_input_neuron  #nociceptive_input_neuron #move_actuator_neuron  0 #rotate_actuator_right_neuron 0 0;;[#xpos #ypos #creature_id #reward_input_neuron #pain_input_neuron #move_neuron #rotate_neuron_left #rotate_neuron_right #pheromone_neuron]
  ;;;;;;;;;;Create Pheromone sensor
  ;create-pheromone-sensor [ #psensor_id #pposition #left_sensor_attached_neuron_label #middle_sensor_attached_neuron_label #right_sensor_attached_neuron_label #attached_creature]
  ;print "created creature"
  ;;;;;;;;;;Create Visual sensors and attach neurons to them;;;;;;;;;
  create-visual-sensor  1 0  white  #visual_input_neuron_white creatureid;[ psensor_id pposition colour_sensitive attached_neuron attached_creature]
  create-visual-sensor  2 0  red  #visual_input_neuron_red creatureid;[ psensor_id pposition colour_sensitive attached_neuron attached_creature]
  create-visual-sensor  3 0  green  #visual_input_neuron_green creatureid;[ psensor_id pposition colour_sensitive attached_neuron attached_creature]
...
to reset_generators_and_recorders
  set_nest_dc_generator 0 0.0
  set_nest_dc_generator 1 0.0
  set_nest_dc_generator 2 0.0
  set_nest_dc_generator 3 0.0
  set_nest_dc_generator 4 0.0
  reset_nest_spike_recorder 0
  reset_nest_spike_recorder 1
end
...
to go
 ...
 [
   ...
   ;do-network-dynamics
    ...
    reset_generators_and_recorders
 ]
 ;if ticks mod 5 = 0 [
 ...
 ;]
 check-boundaries
 update-fitness -0.05;-0.008
 if ticks = 1000 or fitness_value < -10000 [
...
271
Activate DC Gen
(py:run  \n\"nest.SetStatus(activator[0], [{'amplitude': 90000.0}])\"\n)
NIL
1
T
OBSERVER
...
415
Show activity
;set_nest_dc_generator 0 10000.0\nlet spikesr get_nest_spike_recorder 0\nprint spikesr\nreset_nest_spike_recorder 0\n;\"print(nest.GetStatus(middle_spike_detector[0]))\"
NIL
1
T
OBSERVER
