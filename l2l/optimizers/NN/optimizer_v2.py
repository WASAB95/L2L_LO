...
def cluster_data(df, eps, min_samples):
    ...
    clusters = DBSCAN(eps=eps, min_samples=min_samples).fit(clustered_df)
    print(Counter(clusters.labels_))
    clustered_df['cluster'] = clusters.labels_
    return clustered_df, list(set(clusters.labels_)), Counter(clusters.labels_)
...
def sample(seed, num_samples, x, y):
    np.random.seed(seed)
    random_x = np.random.uniform(x[0], x[1], num_samples)
    random_y = np.random.uniform(y[0], y[1], num_samples)
    paired_tuples = np.array([[x_val, y_val] for x_val, y_val in zip(random_x, random_y)])
...
def generate_higher_samples(mean, std, num_samples):
    lower_bound = mean
    std_dev = std

    samples = np.random.normal(mean, std_dev, num_samples)
    samples = [sample for sample in samples if sample > lower_bound]
    return samples

def generate_lower_samples(mean, std, num_samples):
    upper_bound = mean
    std_dev = std

    samples = np.random.normal(mean, std_dev, num_samples)
    samples = [sample for sample in samples if sample < upper_bound]
    return samples

# def generate_samples(mean, std, num_samples):
#     results = []
#     for ind, i in enumerate(sample_direction):
#         if i:
#             results.append(generate_higher_samples(self.best_ind[ind], std, num_samples*2)[:num_samples])
#         else:
#             results.append(generate_lower_samples(self.last_best_ind[ind], std, num_samples*2)[:num_samples])
#     pairs = np.array(list(zip(*results)))
#     return parirs
...
class NNOptimizer(Optimizer):
    def __init__(self, traj,
        ...
        self.last_best_individual = np.array([])
        self.targets = None
        self.final_df = None
        self.counter = 0
        self.current_centroids = None
        self.centroids_list = []
        self.model = Network(self.parameters.neurons).double()
        ...
        self.shift = False
        self.approach = ''
        self.second_best = {'f': -np.inf, 'individuals': []}
        ...
        self.clustered_df, self.clusters_labels, self.cluster_counter = cluster_data(
            self.categorized_df[self.categorized_df['category'] == 1], eps=0.05,
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        if self.g == 0:
            ...
            print(f"fitnesses_list {fitnesses_list}")
            print(f"target labels {target_label}")
            file_prefix = self.cluster_counter[target_label]
            if exists(f'../data_combined_mapped_{file_prefix}.csv'):
                print(f'data_combined_mapped_{file_prefix}.csv exist')
                df = pd.read_csv(f'../data_combined_mapped_{file_prefix}.csv')
            ...
            if self.best_dict[self.g][0] > self.best_individual['f']:
                ...
                self.last_best_individual = self.best_individual['individuals']
                self.best_individual['individuals'] = sorted_population[0]
            else:
                if self.second_best['f'] < self.best_dict[self.g][0]:
                    self.second_best['f'] = self.best_dict[self.g][0]
                    self.second_best['individuals'] = sorted_population[0]
            ...
            if self.g > 1 and (self.best_dict[self.g][0] <= self.best_dict[self.g - 1][0]
                ...
                if abs(self.best_dict[self.g - 1][0] - self.best_dict[self.g][0]) <= 0.01:
                    logger.info(f"diff is: {abs(self.best_dict[self.g - 1][0] - self.best_dict[self.g][0])}")

                self.counter += 0.5

            elif self.best_dict[self.g][0] < 0 and self.best_individual['f'] > 0:
                self.counter += 0.75
            ...
            if self.best_individual['f'] >= self.parameters.target_category:
                if self.g > 1 and (self.best_dict[self.g][0] < self.best_dict[self.g - 1][0]
                                   or abs(self.best_dict[self.g - 1][0] - self.best_dict[self.g][0]) <= 0.001):
                    self.counter += 0.25

            if self.g > 4 and abs(self.best_dict[self.g][0] - self.best_dict[self.g - 2][0]) <= 0.003 and abs(self.best_dict[self.g - 1][0] - self.best_dict[self.g - 3][0]) <= 0.003:
                self.counter += np.inf
            ...
            if 1 < self.g < traj.n_iteration - 1 and self.counter >= count_limit and not stop:
                ...
                #                 #
                #
                #                 if self.g < traj.n_iteration/2:
                #                     filter_collect = collected_data_df[collected_data_df['fitness'] > self.parameters.target_category - 0.1]
                ...
                #
                #                 if len(self.last_best_individual) > 0:
                #                     logger.info("Resampling with 100 STARTED !!!")
                #                     amp_limit = [self.best_individual['individuals'][0], self.last_best_individual[0]]
                #                     phase_limit = [self.best_individual['individuals'][1], self.last_best_individual[1]]
                #                     sample_array = sample(seed=self.g, num_samples=200, x=amp_limit, y=phase_limit)
                #                     logger.info(f"amp sample between {amp_limit}")
                #                     logger.info(f"phase sample between {phase_limit}")
                #
                #                     ref = np.concatenate((sample_array, ref), axis=0)
                #
                # ########
                #
                ...
                #                 #
                #                 self.final_df = pd.concat([self.final_df, collected_data_df], ignore_index=True).drop_duplicates()
                #                 logger.info(f"Best Individual is: {self.best_individual}")
                # #


                # # # ---- NEW shiftting approach ---
                # self.approach = 'new_shifting'
                #
                # self.model.apply(reset_weight)
                #
                #
                # collected_data_df = pd.DataFrame(self.collected_data, columns=np.append(self.targets, 'fitness')).drop_duplicates().dropna()
                # # print(collected_data_df)
                #
                # # percent = 10
                # # numb = int(len(collected_data_df) * (percent / 100)) if (int(len(collected_data_df) * (percent / 100)) > 0) \
                # #     else len(collected_data_df)
                # #
                # # flag = False
                # # if self.parameters.target_category / 2 <= self.best_individual[
                # #     'f'] < self.parameters.target_category - 0.1:
                # #     filter_collect = collected_data_df[
                # #         collected_data_df['fitness'] >= self.parameters.target_category / 2]
                # # elif self.best_individual['f'] < self.parameters.target_category / 2 and self.g > traj.n_iteration/2:
                # #     filter_collect = collected_data_df.sort_values(by=['fitness'], ascending=False).head(numb)
                # #     # if len(self.last_best_individual) > 0:
                # #         # flag = True
                # #
                # # else:
                # #     flag = True
                # #     filter_collect = collected_data_df[
                # #         collected_data_df['fitness'] >= self.parameters.target_category - 0.1]
                # #
                # #
                # # percent = 1
                # # numb = int(len(self.final_df) * (percent / 100)) if (int(len(self.final_df) * (percent / 100)) > 0) else len(self.final_df)
                # #
                # # filter_final = self.final_df[
                # #     self.final_df['fitness'] > self.parameters.target_category].sort_values(by=['fitness'], ascending=False).drop_duplicates() \
                # #     .head(numb)
                # #
                # #
                # # if flag:
                # #     merged_df = filter_collect
                # # else:
                # #     merged_df = pd.concat([filter_collect, filter_final], ignore_index=True)
                # #
                # #
                # # logger.info(f"top {percent}% fitness: {merged_df[['fitness']].values}")
                # #
                # # ref = merged_df.drop(columns=['fitness']).to_numpy()
                #
                # # uniform distrbution
                ...
                # #     amp_limit = [self.best_individual['individuals'][0], self.last_best_individual[0]]
                # #     phase_limit = [self.best_individual['individuals'][1], self.last_best_individual[1]]
                # #     sample_array = sample(seed=self.g, num_samples=100, x=amp_limit, y=phase_limit)
                # #     logger.info(f"amp sample between {amp_limit}")
                # #     logger.info(f"phase sample between {phase_limit}")
                # #     if flag:
                # #         ref = sample_array
                ...
                # # normal distrbution
                #
                # # if self.best_individual['g'] > 1:
                # #     logger.info("Resampling with 1000 STARTED !!!")
                # #     logger.info(f"Flag is {flag}")
                # #     amp_samples = np.random.normal(self.best_individual['individuals'][0], 0.1, 10000)
                # #     phase_samples = np.random.normal(self.best_individual['individuals'][1], 0.1, 10000)
                # #     sample_array = np.array([[x_val, y_val] for x_val, y_val in zip(amp_samples, phase_samples)])
                # #     ref = sample_array
                #
                ...
                # # if len(self.last_best_individual) > 0:
                # #     logger.info("Resampling with 100 STARTED !!!")
                # #     logger.info(f"previous best individual: {self.last_best_individual}")
                # #     logger.info(f"current best individual: {self.best_individual['individuals']}")
                # #     sample_direction = np.subtract(self.best_individual['individuals'], self.last_best_individual) > 0
                # #     results = []
                # #     for ind, i in enumerate(sample_direction):
                # #         if i:
                # #             results.append(generate_higher_samples(self.best_individual['individuals'][ind], std, num_samples*2)[:num_samples])
                # #         else:
                # #             results.append(generate_lower_samples(self.last_best_individual[ind], std, num_samples*2)[:num_samples])
                # #
                # #     ref = np.array(list(zip(*results)))
                # #
                # # else:
                # #     logger.info(f"Resample {num_samples} samples from the best only ")
                # #     ref = sample_normal(seed=self.g, std=std, x=self.best_individual['individuals'], num_samples=num_samples)
                # # self.approach = 'shifting_custom_sample'
                # ############################################
                # logger.info(f"Resample {num_samples} samples from the best only ")
                # ref = sample_normal(seed=self.g, std=std, x=self.best_individual['individuals'], num_samples=num_samples)
                # self.approach = 'shifting_normal_sample'
                # logger.info(f" ref is: {ref[:10]}")
                # #################################
                #
                # collected_data_df[self.targets] = collected_data_df.apply(
                #     lambda row: self.min_distance(row=row, ref_array=ref), axis=1,
                #     result_type='expand')
                #
                # if len(self.training_generations) > 3 and self.best_dict[1][0] == self.best_individual['f']:
                #     self.centroids_list.append(self.second_best['individuals'])
                #     logger.info(f"second best fitness is : {self.second_best['f']}")
                #
                # # if len(self.training_generations) == 0:
                # #     self.centroids_list = [self.best_individual['individuals']]
                # # else:
                ...
                # if self.g > traj.n_iteration/2 and self.best_individual['f'] < self.parameters.target_category:
                #     target_individuals = collected_data_df[self.targets][
                #         collected_data_df['fitness'] >= self.parameters.target_category/2].drop_duplicates().to_numpy()
                # else:
                #     target_individuals = collected_data_df[self.targets][
                ...
                if np.any(target_individuals):
                    ...
                    best_ind = np.array(self.best_individual['individuals'])
                ...
                if len(self.training_generations) < 1:
                    ...
                    num_samples = int(len(self.final_df)*0.2)
                    ...
                    num_samples = int(len(collected_data_df)*0.2)
                    ...
                    logger.info(f"centroids fitness {self.best_individual['f']}")
                    logger.info(f"number of centroids {len(self.centroids_list)}")
                    logger.info(f"centroid list: {self.current_centroids}")
                    adjust = np.mean(self.centroids_list, axis=0) - self.current_centroids
                    self.current_centroids = np.mean(self.centroids_list, axis=0)
                    # logger.info(f"new centroids: {self.current_centroids}")
                ...
                #
                # logger.info(f"Resample {num_samples} samples from the best only {self.best_individual['f']}")
                # ref = sample_normal(seed=self.g, std=std, x=best_ind,
                #                     num_samples=num_samples)
                #
                #
                ...
                #################################
                # LAST TRY
                # self.approach = 'last_try_approach'
                # self.model.apply(reset_weight)
                # collected_data_df = pd.DataFrame(self.collected_data, columns=np.append(self.targets, 'fitness')).drop_duplicates().dropna()
                #
                # std = 0.2
                ...
                # logger.info(f"number of centroids {len(self.centroids_list)}")
                # # adjust = np.mean(self.centroids_list, axis=0) - self.current_centroids
                # adjust = math.dist(np.mean(self.centroids_list, axis=0), self.current_centroids)
                # self.current_centroids = np.mean(self.centroids_list, axis=0)
                # logger.info(f"new centroids: {self.current_centroids}")
                #
                # self.final_df[self.targets] = self.final_df[self.targets] +  adjust
                ...
                #     print(collected_data_df)
                #
                #     filter_collect = collected_data_df[collected_data_df['fitness'] > self.parameters.target_category - 0.1].sort_values(by=['fitness'], ascending=False)
                #     filter_final = self.final_df[self.final_df['fitness'] > self.parameters.target_category - 0.1].sort_values(by=['fitness'], ascending=False)
                #
                #     # filter_collect = collected_data_df[collected_data_df['fitness'] > self.parameters.target_category - 0.1]
                #     # filter_final = self.final_df[self.final_df['fitness'] > self.parameters.target_category - 0.1]
                #
                #
                #     merged_df = pd.concat([filter_collect, filter_final], ignore_index=True)
                #
                #     # merged_df = merged_df.sort_values(by=['fitness'], ascending=False).drop_duplicates().head(int(len(merged_df) * (precent / 100)))
                #     merged_df = merged_df.drop_duplicates().head(int(len(merged_df) * (precent / 100)))
                #     logger.info(f"top {precent}% fitness: {merged_df[['fitness']].values}")
                #
                #     ref = merged_df.drop(columns=['fitness']).to_numpy()
                #
                ...
                #     self.final_df = pd.concat([self.final_df, collected_data_df], ignore_index=True)
                #
                #     filter_new_df = self.final_df[self.final_df['fitness'] > self.parameters.target_category - 0.1]
                ...
                ### NEW Re-mapping approach ##########
                # precent = 10
                # self.approach = 'new_ReMapping'
                # self.model.apply(reset_weight)
                #
                # collected_data_df = pd.DataFrame(self.collected_data, columns=np.append(self.targets, 'fitness')).drop_duplicates()
                # filter_final_df = self.final_df[self.final_df['fitness'] > self.parameters.target_category].sort_values(by=['fitness'], ascending=False)
                #
                # self.final_df = pd.concat([self.final_df, collected_data_df], ignore_index=True)
                #
                # filter_collected = collected_data_df[collected_data_df['fitness'] > self.parameters.target_category - 0.1]
                #
                #
                # ref_len = int(len(self.final_df)*precent/100)
                #
                # ref = pd.concat([filter_collected, filter_final_df.head(ref_len-len(filter_collected))], ignore_index=True)
                #
                # logger.info(f"top {precent}% fitness: {ref[['fitness']].values}")
                # ref = ref.drop(columns=['fitness']).to_numpy()
                #
                ...
                #
                #     num = 20
                #     precent = num if (int(len(collected_data_df) * (num / 100)) > 0) else 100
                #
                #     if self.parameters.target_category / 2 <= self.best_individual[
                #         'f'] < self.parameters.target_category - 0.1:
                #         filter_collect = collected_data_df[
                #             collected_data_df['fitness'] >= self.parameters.target_category / 2]
                #     elif self.best_individual['f'] < self.parameters.target_category / 2 and self.g > traj.n_iteration/2:
                #         filter_collect = collected_data_df.sort_values(by=['fitness'], ascending=False).drop_duplicates().head(
                #             int(len(self.final_df) * (precent / 100)))
                ...
                #
                #     num = 5
                #     precent = num if (int(len(self.final_df) * (num / 100)) > 0) else 100
                #
                #     filter_final = self.final_df[
                #         self.final_df['fitness'] > self.parameters.target_category].sort_values(by=['fitness'],                                                                        ascending=False).drop_duplicates() \
                #         .head(int(len(self.final_df) * (precent / 100)))
                #     print("Count of final > 0,8")
                #     print(len(filter_final))
                #
                #     merged_df = pd.concat([filter_collect, filter_final], ignore_index=True)
                #
                #
                ...
                #     else:
                #         filter_collect = collected_data_df[
                #             collected_data_df['fitness'] >= self.parameters.target_category]
                #
                #     num = 5
                #     precent = num if (int(len(self.final_df) * (num / 100)) > 0) else 100
                #
                #
                #     filter_new_df = old_final_df[old_final_df['fitness'] > self.parameters.target_category].sort_values(by=['fitness'], ascending=False).drop_duplicates().head(
                #         int(len(old_final_df) * (precent / 100)))
                #
                #     merged_df = pd.concat([filter_collect, filter_new_df], ignore_index=True)
                #
                #     logger.info(f"top {precent}% fitness: {merged_df[['fitness']].values}")
                #
                #     ref = merged_df.drop(columns=['fitness']).to_numpy()
                ...
                train_loader = self.get_train_loader(fitness, targets, self.parameters.batch_size)
                logger.info(f"Training Started {len(self.training_generations) + 1} ...")
                self.train_network(train_loader, self.parameters.epochs)
    ...
    def end(self, traj):
        ...
        # pd.DataFrame(self.best_dict).T.to_csv("/home/wasab/final_results/SG/all_individuals_{}".format(datetime.now().strftime("%Y-%m-%d-%H_%M_%S")) + '.csv')
        pd.DataFrame(self.best_dict).T.to_csv(f'/home/wasab/final_results/SG/{self.best_individual["f"]}_{self.best_individual["g"]}_all_individuals_{format(datetime.now().strftime("%Y-%m-%d-%H_%M_%S"))}.csv')
        ...
        plt.scatter(self.training_generations, [mu[i-1] for i in self.training_generations], color='r', label='Optimizer Training', s=50)
        plt.xlabel('Generations')
        plt.ylabel('Fitness')
        plt.legend()
        plt.grid(True)
        avg_fit = np.mean([i[0] for i in self.best_dict.values()])
        logger.info(f"avg of all best is: {avg_fit}")
        plt.savefig(f'/home/wasab/final_results/SG/fitness_{self.parameters.learning_rate}_{self.parameters.epochs}_{self.parameters.pop_size}_{self.parameters.n_iteration}_{self.approach}_{avg_fit}_{self.best_individual["f"]}_{self.best_individual["g"]}_{time.time()}.pdf', bbox_inches='tight', pad_inches=0.1)
        plt.show()
    ...
    def get_train_data(self, df, clustered_df, target_label):
        ...
        data[self.targets] = data.apply(lambda row: self.min_distance(row=row, ref_array=ref), axis=1,
                                        result_type='expand')
        # data.to_csv('data_combined_mapped_2887.csv', index=False)
        self.final_df = data
    ...
    #     # x_percent = 10
    #     # logger.info(f"data mapped to the neart {one_percent} points")
    ...
    def min_distance(self, row, ref_array):
        ...
        percent = 0.4
        x_percent = round(len(ref_array) * percent / 100) if round(len(ref_array) * percent / 100) > 1 else 5
        # x_percent = 50
