...
class EnsembleKalmanFilter(Optimizer):
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        # separation into excitatory and inhibitory weights
        weights_e = [np.concatenate(
        ...
        if not self.g % 10 == 0 and self.g >= 0:
            if traj.sample:
                ...
                ens_e, model_outs = self.sample_from_individuals(
                    individuals=ens_e,
                    ...
                    **traj.kwargs)
                ens_i, _ = self.sample_from_individuals(individuals=ens_i,
                                                        model_output=model_outs,
                                                        fitness=fitness,
                                                        sampling_method=traj.sampling_method,
                                                        pick_method=traj.pick_method,
            ...
            enkf.fit(ensemble=ens_e,
                     ensemble_size=ensemble_size,
                     observations=np.array(self.optimizee_labels),
                     model_output=model_outs,
                     gamma=gamma)
            # These are all the updated weights for each ensemble
            results_e = enkf.ensemble.cpu().numpy()
            self.cov_e = enkf.cov_mat
            # clear for inhibitory run
            enkf.clear(maxit=traj.maxit,
                       online=traj.online,
                       n_batches=len(self.optimizee_labels))
            # now run for inhibitory weights
            results_i = enkf.fit(ensemble=ens_i,
                                 ensemble_size=ensemble_size,
                                 observations=np.array(self.optimizee_labels),
                                 model_output=model_outs,
                                 gamma=gamma)
            results_i = enkf.ensemble.cpu().numpy()
            self.cov_i =  enkf.cov_mat
        else:
            results_e = ens_e
            results_i = ens_i
        ...
        if traj.scale_weights:
            ...
            results_e = results_e * np.abs(weights_e).max()
            results_i = results_i * np.abs(weights_i).max()
            results_i = np.abs(results_i) * -1
            # scaler.inverse_transform(results)
        # self.plot_distribution(weights=results, gen=self.g, mean=True)

        if self.g % 1 == 0 and self.g > 0:
            self.weights_to_save.append(np.hstack((results_e, results_i)))
        generation_name = 'generation_{}'.format(traj.generation)
        traj.results.generation_params.f_add_result_group(generation_name)

        # now concatenate results_e and results_i to one results list
        results = np.hstack((results_e, results_i))
        ...
        if traj.stop_criterion <= self.current_fitness or self.g < traj.n_iteration:
            ...
            # Apply test set
                ...
                self.cov_to_save['ex'].append(self.cov_e)
                self.cov_to_save['inh'].append(self.cov_i)
    ...
    def end(self, traj):
        ...
        np.savez_compressed(os.path.join(traj.parameters.path, 'covariance_matrices_e.npz'),
                            covmat=self.cov_to_save['ex'])
        np.savez_compressed(os.path.join(traj.parameters.path, 'covariance_matrices_i.npz'),
                           covmat=self.cov_to_save['inh'])
